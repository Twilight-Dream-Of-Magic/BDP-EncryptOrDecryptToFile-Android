...
public class FileCustomAlgorithmCipherUtil
{
	...
	public static boolean runEncryptFile (String sourceFile, byte[] encryptKey, byte[] encryptKey2, byte[] encryptKey3, byte[] encryptKey4, String targetFile, CipherListener listener) throws FileNotFoundException, IOException
	{
		try
		{
					...
					for(int start_group = 0, stop_group = 256 ; start_group <= stop_group ; start_group++)
					{
						...
						while( readFileToGetBytesSize != -1 )
						{
							for( int count = 0, index = 0; count <= readFileToGetBytesSize; count++, index++ )
							{
								...
								baos.write(temporaryBuffer,0,readFileToGetBytesSize);
							}
							byte[] fileBytesBuffer = baos.toByteArray(); 
							fos.write(fileBytesBuffer);
							readFileToGetBytesSize = -1;
						}
						fos.flush();
						fos.close();
						baos.flush();
		...
		{
			Logger.getLogger(FileCustomAlgorithmCipherUtil.class.getName()).log(Level.SEVERE,null,IO_Exception);
			return false;
		}
	...
	public static boolean runDecryptFile (String sourceFile, byte[] decryptKey, byte[] decryptKey2, byte[] decryptKey3, byte[] decryptKey4, String targetFile, CipherListener listener) throws FileNotFoundException, IOException
	{
		try
		{
					...
					*/
					...
					for(int start_group = 0, stop_group = 256 ; start_group <= stop_group ; start_group++)
					{
						...
						while( readFileToGetBytesSize != -1 )
						{
							...
							byte[] fileByteBuffer = baos.toByteArray();
							fos.write(fileByteBuffer);
							readFileToGetBytesSize = 1;
						}
						fos.flush();
						fos.close();
